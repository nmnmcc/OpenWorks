...
export class Auth extends Context.Service<Auth>()(
	"@openworks/frontend/services/auth/Auth",
	{
		make: Effect.gen(function* () {
			...
			type API = {
				[K in keyof typeof auth.api]: (typeof auth.api)[K] extends (
					...args: infer A
				) => PromiseLike<infer R>
					? (
							...args: A
						) => Effect.Effect<R, Auth.Error.API | Auth.Error.Unknown>
					: never;
			};
						...
						Effect.tryPromise({
							try: () => endpoint(params),
								...
								cause instanceof APIError
									? new Auth.Error.API({ cause })
									: new Auth.Error.Unknown({ cause }),
						}),
				) as unknown as API,
			};
		}),
