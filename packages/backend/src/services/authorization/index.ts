...
type BuildAbilityError =
	| EffectDrizzleQueryError
	| Ability.AliasError
	| Ability.RawRuleError;
	...
	"@openworks/backend/services/authorization/Authorization",
	{
		make: Effect.gen(function* () {
			...
			const buildAbility = (
				userId: string,
				groupId: string,
			): Effect.Effect<AuthorizationAbility, BuildAbilityError> =>
				Effect.gen(function* () {
					const rows = yield* db
						.select({
						...
						.innerJoin(
							rolePermissions,
							eq(groupMembers.roleId, rolePermissions.roleId),
						)
						.innerJoin(
							permissions,
			...
			return {
				...
				check: (
					...
					buildAbility(userId, groupId).pipe(
						Effect.flatMap((ability) => Ability.check(ability, request)),
					),
				...
				permittedFields: (
					...
					buildAbility(userId, groupId).pipe(
						Effect.flatMap((ability) =>
							Ability.permittedFields(ability, request, options),
						),
					),
			} as const;
		}),
